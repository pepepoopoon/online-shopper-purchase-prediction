...
`make experiment` сравнивает все модели на одном стратифицированном разбиении и
сохраняет параметры, validation-метрики, выбранную модель и test-метрики в JSON.
Параметр `--baseline` добавляет дельты основных test-метрик относительно ранее
зафиксированного запуска. Параметр `--missing-rate` задаёт контролируемую долю
пропусков для проверки устойчивости предобработки, а `--unseen-category-rate`
подменяет часть категорий только после split и проверяет безопасный инференс на новых значениях.
Каждый результат также содержит диагностические срезы test-очереди по типу посетителя и месяцу.
Выполнены 76 запусков; условия, агрегированная сводка и ограничения описаны в
[`experiments/README.md`](experiments/README.md).
Результаты на синтетических данных используются только для проверки устойчивости
pipeline и не выдаются за качество на исходном датасете UCI.
